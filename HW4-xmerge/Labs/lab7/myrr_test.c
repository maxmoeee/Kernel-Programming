...
/* In user-space, SCHED_MYRR is not defined in GNU library */
#define SCHED_MYRR 9 
...
int main(int argc, const char *argv[])
{
	...
	for (i = 0; i<num_process; i++) {
		...
		if ( pid == 0 ) { // child process
			...
			ret = sched_setscheduler( pid, SCHED_MYRR, &sp );
			if ( ret == -1 ) {
